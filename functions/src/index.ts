...
export const createInvitationCode = functions.https.onCall(async (data, context) => {
...
export const checkInvitationCode = functions.https.onCall(async (data, context) => {
    if (context.auth?.uid === undefined) {
        throw new functions.https.HttpsError('failed-precondition', 'ログインをしてください')
    }
...
export const createInvitationReview = functions.https.onCall(async (data, context) => {
    if (context.auth?.uid === undefined) {
        throw new functions.https.HttpsError('failed-precondition', 'ログインをしてください')
    }
    const clientToken: string = data.token
...
export const getBookTree = functions.https.onCall(async (data: getBookTreePostType, context) => {
    const userID = data.userID
    if (!userID) {
        throw new functions.https.HttpsError('invalid-argument', 'userIDが必要です。')
    }
    ...
    const isTo = admin
        ...
        .where('accepted', '==', true)
        .get()
    const [querySnapshotFrom, querySnapshotTo] = await Promise.all([isFrom, isTo])
