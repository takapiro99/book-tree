...
const CreateLink = () => {
    ...
    return <div className={globalStyles.wrapper}>
        ...
        <div className={styles.createLinkBlock}>
            ...
            <div className={styles.createLinkBlock__input}><input className={styles.createLinkBlock__reason} placeholder="なにがすごい？" value={reason} onChange={handleChange}/><span>がすごいひとにお願いする</span></div>
            <div className={styles.createButtonBlock}><button className={styles.createButtonBlock__button} onClick={makeLink}>作成</button></div>
        </div>{reason}
        {/* { addLink?.map((link) => {
